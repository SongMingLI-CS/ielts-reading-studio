...
11. **FastAPI 管理页面**
    ...
    - 材料库页每份材料显示进度、状态、解析置信度、源文件是否仍在原路径，并给出下一步按钮；章节解析页支持分页、跳页、每页条数，以及按行触发的重命名/合并/拆分（不再要求手写 JSON，JSON 端点保留为高级入口）；解析诊断码以中文说明展示。
    - 证据：`tests/web/test_corpora.py`、`tests/web/test_jobs.py`，以及本地 Uvicorn 200 响应烟雾检查。

12. **浏览器练习、评分、解析与导出中心**
    - 练习页不渲染答案；草稿与成绩都落到 SQLite，刷新、换浏览器后可继续。
    - 提交答案同时支持 JSON 与普通表单两种报文：页面脚本失败或浏览器禁用 JavaScript 时，表单回退仍会判分、保存并跳转到结果页。
