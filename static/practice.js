(()=>{
  ...
  const updateProgress=()=>{const answers=collect(),count=Object.keys(answers).length,total=numbers.length,percent=total?count/total*100:0;answeredCount.textContent=count;totalCount.textContent=total;progressCopy.textContent=`已完成 ${count} / ${total} 题`;progressBar.style.width=`${percent}%`;for(const block of form.querySelectorAll('.question'))block.classList.toggle('answered',Boolean(answers[block.dataset.number]))};
