...
def export_chapter_html(chapter: ConvertedChapter, path: str | Path, *, cumulative: dict[str, int] | None = None) -> Path:
    ...
    for paragraph in chapter.paragraphs:
        ...
        for term in sorted(paragraph.inserted_terms, key=lambda value: len(value.word), reverse=True):
            ...
            # Bind the loop-local values as defaults: the replacement callbacks run inside this
            # iteration today, and binding keeps them correct if the call is ever moved or deferred.
            ...
            if not replaced:
                rendered = re.sub(
                    rf"\b{re.escape(html.escape(term.word))}\b",
                    lambda _match, span=span: span,
                    rendered,
                    count=1,
                )
